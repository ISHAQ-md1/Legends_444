...
const BOT_NAME_FANCY = '—͟͞͞★ɪsʜᴀǫ-ᴍᴅㅤ✓';
...
// ========== END WORK TYPE RESTRICTIONS ==========
    ...
    let botName = cfg.botName || '—͟͞͞★ɪsʜᴀǫ-ᴍᴅㅤ✓';
  ...
  paramsJson: JSON.stringify({
    ...
    sections: [

      {
        title: '🔵 Work Mode',
        rows: [
          { title: 'Public', description: 'Bot works everywhere', id: `${prefix}wtype public` },
          { title: 'Groups Only', description: 'Only groups', id: `${prefix}wtype groups` },
          { title: 'Inbox Only', description: 'Only private chats', id: `${prefix}wtype inbox` },
          { title: 'Private Only', description: 'Owner only', id: `${prefix}wtype private` },
        ],
      },

      {
        ...
        rows: [
          { title: 'Typing ON 🔵', description: 'Show typing...', id: `${prefix}autotyping on` },
          { title: 'Typing OFF ⚪', description: 'Disable typing', id: `${prefix}autotyping off` },
        ],
      },

      {
        ...
        rows: [
          { title: 'Recording ON 🔵', description: 'Show recording...', id: `${prefix}autorecording on` },
          { title: 'Recording OFF ⚪', description: 'Disable recording', id: `${prefix}autorecording off` },
        ],
      },

      {
        title: '🔵 Presence',
        rows: [
          { title: 'Always Online 🔵', description: 'Stay online', id: `${prefix}botpresence online` },
          { title: 'Offline ⚪', description: 'Hide presence', id: `${prefix}botpresence offline` },
        ],
      },

      {
        ...
        rows: [
          { title: 'Auto Seen 🔵', description: '', id: `${prefix}rstatus on` },
          { title: 'Auto Seen OFF ⚪', description: '', id: `${prefix}rstatus off` },
          { title: 'Auto React 🔵', description: '', id: `${prefix}arm on` },
          { title: 'Auto React OFF ⚪', description: '', id: `${prefix}arm off` },
        ],
      },

      {
        title: '🔵 Protection',
        rows: [
          { title: 'Reject Calls 🔵', description: '', id: `${prefix}creject on` },
          { title: 'Reject Calls OFF ⚪', description: '', id: `${prefix}creject off` },
        ],
      },

      {
        title: '🔵 Message Read',
        rows: [
          { title: 'Read All 🔵', description: '', id: `${prefix}mread all` },
          { title: 'Commands Only', description: '', id: `${prefix}mread cmd` },
          { title: 'Read OFF ⚪', description: '', id: `${prefix}mread off` },
        ],
      }
...
case 'settings': {
  try {
    ...
    // build native select flow (same as /setting)
const settingOptions = {
  name: 'single_select',
  paramsJson: JSON.stringify({
    title: `🔷 ${botName} Settings`,
...
▫️ _Work Type_   : \`${currentConfig.WORK_TYPE || 'public'}\`
▫️ _Presence_    : \`${currentConfig.PRESENCE || 'available'}\`
▫️ _Status Seen_ : ${dot(currentConfig.AUTO_VIEW_STATUS)}
▫️ _Status React_: ${dot(currentConfig.AUTO_LIKE_STATUS)}
▫️ _Reject Call_ : ${dot(currentConfig.ANTI_CALL)}
▫️ _Auto Read_   : ${dot(currentConfig.AUTO_READ_MESSAGE)}
▫️ _Recording_   : ${dot(currentConfig.AUTO_RECORDING)}
▫️ _Typing_      : ${dot(currentConfig.AUTO_TYPING)}
▫️ _Prefix_      : \`${currentConfig.PREFIX || '.'}\`

🔷 _Tap below to configure settings_`;
    ...
    await socket.sendMessage(sender, {
      ...
      footer: botName,
      buttons: [
        { buttonId: `${prefix}wtype`, buttonText: { displayText: '🛠️ Work Type' }, type: 1 },
        { buttonId: `${prefix}autotyping on`, buttonText: { displayText: '⌨️ Typing ON' }, type: 1 },
        { buttonId: 'settings_action', buttonText: { displayText: '⚙️ Configure' }, type: 4, nativeFlowInfo: settingOptions }
      ]
    }, { quoted: msg });
 ...
 case 'weather':
    try {
        ...
        const messages = {
            ...
            weather: (data) => `
*☘️ —͟͞͞★ɪsʜᴀǫ-ᴍᴅㅤ✓ 𝐖eather 𝐑eport ☘️*
...
case 'menu': {
  ...
  try {
    ...
    const title = userCfg.botName || '—͟͞͞★ɪsʜᴀǫ-ᴍᴅㅤ✓';
...
`.trim();
    ...
    const dateStr = now.toLocaleDateString();
    const timeStr = now.toLocaleTimeString();
    const logo = userCfg.logo || config.RCD_IMAGE_PATH;
    
    const caption = `╭━━━─  *📥 DOWNLOAD MENU*  ─━━╮\n│ 📅 Date: ${dateStr}\n│ ⏰ Time: ${timeStr}\n├─────────────────────────\n│ 🎵 Music • 🎬 Video\n│ 📱 Apps • 📁 Files\n╰─────────────────────────\n\nTap buttons for quick access.`;

    const buttons = [
      { buttonId: `${config.PREFIX}song sample`, buttonText: { displayText: "🎵 MUSIC" }, type: 1 },
      { buttonId: `${config.PREFIX}tiktok`, buttonText: { displayText: "🎬 VIDEO" }, type: 1 },
      { buttonId: `${config.PREFIX}apk app`, buttonText: { displayText: "📱 APPS" }, type: 1 },
...
`.trim();
    ...
    const caption = `╭━━━─  *🎨 CREATIVE MENU*  ─━━╮\n│ 📅 Date: ${dateStr}\n│ ⏰ Time: ${timeStr}\n├─────────────────────────\n│ 🤖 AI Tools • ✍️ Text\n│ 🖼️ Images • 💾 Media\n╰─────────────────────────\n\nTap buttons for quick access.`;

    const buttons = [
      { buttonId: `${config.PREFIX}ai hello`, buttonText: { displayText: "🤖 AI CHAT" }, type: 1 },
      { buttonId: `${config.PREFIX}aiimg nature`, buttonText: { displayText: "🖼️ AI IMAGE" }, type: 1 },
      { buttonId: `${config.PREFIX}font hello`, buttonText: { displayText: "✍️ FONTS" }, type: 1 },
      { buttonId: `${config.PREFIX}menu`, buttonText: { displayText: "📄 MENU" }, type: 1 },
...
`.trim();
    ...
    const caption = `╭━━━─  *🛠️ TOOLS MENU*  ─━━╮\n│ 📅 Date: ${dateStr}\n│ ⏰ Time: ${timeStr}\n├─────────────────────────\n│ 🆔 Info • 👥 Groups\n│ 🔍 Search • 📊 Status\n╰─────────────────────────\n\nTap buttons for quick access.`;

    const buttons = [
      { buttonId: `${config.PREFIX}jid`, buttonText: { displayText: "🆔 GET JID" }, type: 1 },
      { buttonId: `${config.PREFIX}ping`, buttonText: { displayText: "📡 PING" }, type: 1 },
      { buttonId: `${config.PREFIX}google search`, buttonText: { displayText: "🔍 SEARCH" }, type: 1 },
      { buttonId: `${config.PREFIX}menu`, buttonText: { displayText: "📄 MENU" }, type: 1 },
...
END:VCARD` } }
        ...
        const buttonMessage = {
            ...
            footer: config.FOOTER || '> *—͟͞͞★ɪsʜᴀǫ-ᴍᴅㅤ✓*',
            buttons: buttons,
             headerType: 4,
            contextInfo: { mentionedJid: [sender] }
        };
...
case 'jid': {
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || '—͟͞͞★ɪsʜᴀǫ-ᴍᴅㅤ✓'; // dynamic bot name
